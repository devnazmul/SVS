...
export default function ManageAudience({ tabName }) {
  const { user } = useAuth();
  // DATA FOR MULTI
  ...
  if (isGettingData || isLoadingRoles || isLoadingUsers) {
    ...
    return (
      <div className="w-full flex flex-col lg:flex-row items-center lg:items-start justify-center gap-10 pb-10">
        <div className="w-full sm:w-full lg:w-1/2 lg:rounded-xl lg:shadow-xl p-5 lg:shadow-primary-content lg:border lg:border-gray-300">
          ...
          >
            <p>
              <span className="font-medium -ml-5">Restriction Note:</span>
              <ul className="list-decimal">
                <li>
                  Create badge for allowance or deduction from Beneficiary badge
                  module.
                </li>
                <li>
                  Select badge and assign a value that will applicable for all
                  employees (Except those are updated individually) while
                  execute payrun.
                </li>
              </ul>
            </p>
          ...
          <div>
            <div className="mt-5 text-xl font-bold">Restricted User (For Payrun)</div>
            {/* FOR Department preference  */}
            <CustomMultiSelect
            ...
            />
            <div className="mt-5 text-xl font-bold ">Restricted User (For Beneficiary Badge)</div>
            {/* FOR Employment Status preference  */}
            <CustomMultiSelect
              error={errors?.special_users}
              options={status}
              top
              label={"Employment Status preference"}
              required={true}
              defaultSelectedValues={status.filter((item2) =>
                formData2.unpaid_leave_employment_statuses.some(
                  (item1) => item1 === item2.id
