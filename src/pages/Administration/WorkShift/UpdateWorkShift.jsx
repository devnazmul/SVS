...
export default function UpdateWorkShift({ handleClosePopup, id }) {
  const { user } = useAuth();
  ...
  const [isLoadingBusinessTiming, setIsLoadingBusinessTiming] = useState(true);
      ...
      if (formData.type === "regular") {
        setFormData({
          ...
          details: formData.details.map((d) => ({
            ...d,
            start_at: businessTiming?.find((bt) => bt?.day === d?.day)
              ?.is_weekend
              ? ""
              : lowestTime || "",
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-5 px-5 mb-2">
                  {[
                    ...
                    <div>
                      {console.log({
                        is_W: businessTiming.details.find(
                          (d) => d.day === day?.value
                        ).is_weekend,
                      })}
                      {/* DAY TITLE  */}
